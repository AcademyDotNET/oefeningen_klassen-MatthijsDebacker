...
namespace PokemonsVergelijken
{
    class Pokemon
    {
        ...
        {
            if (obj == null || obj.GetType().BaseType != typeof(Pokemon) && obj.GetType() != typeof(Pokemon))
            {
                return false;
            }
            else if (Level == ((Pokemon)obj).Level && Name == ((Pokemon)obj).Name)
            {
                Pokemon pok = (Pokemon)obj;
                if(HP_Base == pok.HP_Base 
                    && Attack_Base == pok.Attack_Base
                    && Defence_Base == pok.Defence_Base
                    && SpecialAttack_Base == pok.SpecialAttack_Base
                    && SpecialDefence_Base == pok.SpecialDefence_Base
                    && Speed_Base == pok.Speed_Base)
                {
                    return true;
                }
            }
        ...
        {
            string info = "";
            info += $"#{Number}: {Name} (level {Level}) (Type: {Type})\n";
            info += "Base stats:\n";
            info += $"\t* Health = {HP_Base}\n";
            info += $"\t* Attack = {Attack_Base}\n";
            info += $"\t* Defence = {Defence_Base}\n";
            info += $"\t* Special Attack = {SpecialAttack_Base}\n";
            info += $"\t* Special Defence = {SpecialDefence_Base}\n";
            info += $"\t* Speed = {Speed_Base}\n";
            info += "Full stats:\n";
            info += $"\t* Health = {HP_Full}\n";
            info += $"\t* Attack = {Attack_Full}\n";
            info += $"\t* Defence = {Defence_Full}\n";
            info += $"\t* Special Attack = {SpecialAttak_Full}\n";
            info += $"\t* Special Defence = {SpecialDefence_Full}\n";
            info += $"\t* Speed = {Speed_Full}\n";
